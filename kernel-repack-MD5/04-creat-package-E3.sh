...
if [ -f run.sh ] && [ -f md5 ]; then
	...
	cd ..
	tar -cvf E3-PDA-boot.img.tar boot.img && mv boot.img kernel/ && cd kernel && zip -r ../E3-boot.img.zip . ;
	cd ..
	mv E3-boot.img.zip ../$(cat ../common/.version)-E3-boot.img.zip && echo "$(cat ../common/.version)-E3-boot.img.zip <-> READY"
	mv E3-PDA-boot.img.tar ../$(cat ../common/.version)-E3-PDA-boot.img.tar && echo "$(cat ../common/.version)-E3-PDA-boot.img.tar <-> READY"
else
