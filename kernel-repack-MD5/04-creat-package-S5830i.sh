...
if [ -f run.sh ] && [ -f md5 ]; then
	...
	cd ..
	tar -cvf S5830i-PDA-boot.img.tar boot.img && mv boot.img kernel/ && cd kernel && zip -r ../S5830i-boot.img.zip . ;
	cd ..
	mv S5830i-boot.img.zip ../$(cat ../common/.version)-S5830i-boot.img.zip && echo "$(cat ../common/.version)-S5830i-boot.img.zip <-> READY"
	mv S5830i-PDA-boot.img.tar ../$(cat ../common/.version)-S5830i-PDA-boot.img.tar && echo "$(cat ../common/.version)-S5830i-PDA-boot.img.tar <-> READY"
else
